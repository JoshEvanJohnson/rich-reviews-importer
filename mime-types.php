 ...
 *
 * Make sure the WordPress install will accept .rri uploads.
 *
 * @since 0.1
 * @param array $mime_types Currently uploadable mime types.
 * @return array Mime types with additions
 */
function rri_add_mime_types( $mime_types ) {

	$mime_types['rri'] = 'application/json';

	return $mime_types;

}

add_filter( 'upload_mimes', 'rri_add_mime_types' );
 ...
 *
 * This is called in includes/import.php by rri_upload_import_file() so that it only happens during upload via this
 * plugin. add_filter( 'wp_check_filetype_and_ext', 'rri_disable_real_mime_check', 10, 4 );
 *
 ...
 * https://wordpress.org/support/topic/solution-for-wp-4-7-1-bug-causing-you-must-upload-a-rri-file-generated-by/
 */
function rri_disable_real_mime_check( $data, $file, $filename, $mimes ) {
